const FOLLOWUP_MESSAGES = [
  ...
  },
  {
    ...
    text: "Добрый день! Прошла неделя. Я не пишу чтобы давить — просто хочу оставить вам наш контакт. Когда будете готовы масштабировать бизнес — SPECTO к вашим услугам. Работаем только за результат, 10% с продаж. Удачи в бизнесе! 🤝"
  }
];
  ...
  FOLLOWUP_MESSAGES.forEach((msg, index) => {
    const timer = setTimeout(async () => {
      if (followUpTimers[chatId]) {
        console.log("Дожим " + (index + 1) + " для " + chatId);
...
export function cancelFollowUp(chatId) {
  if (followUpTimers[chatId]) {
    followUpTimers[chatId].forEach(timer => clearTimeout(timer));
